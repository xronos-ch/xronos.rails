SELECT 
  ...
  COALESCE((
	...
	JOIN contexts ON sam.context_id = contexts.id
	JOIN samples AS all_samples ON all_samples.context_id = contexts.id
	WHERE all_samples.id = samples.id
  ), '[]'::json) as periods,
  COALESCE((
  ...
  JOIN samples AS all_samples ON all_samples.context_id = contexts.id
  WHERE all_samples.id = samples.id
  ), '[]'::json) as typochronological_units,
  COALESCE((
  ...
  JOIN samples AS all_samples ON all_samples.context_id = contexts.id
  WHERE all_samples.id = samples.id
  ), '[]'::json) as ecochronological_units,
  COALESCE((
