...
class ControlledVocabulariesControllerTest < ActionDispatch::IntegrationTest
  ...
  def smoke_vocabulary_name
    @smoke_vocabulary_name ||= create(:controlled_vocabulary).name
  end
  ...
  test 'index returns multiple terms when several variants match' do
    create(:controlled_vocabulary_variant, term: @femur, value: 'thigh bone')
